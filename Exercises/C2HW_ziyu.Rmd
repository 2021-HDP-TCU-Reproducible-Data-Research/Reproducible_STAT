...
```{r Q4}
Infant_Mortal <- read.csv("../data/infmort.csv")
```

再一次使用`readr::read_csv()`匯入"infmort.csv"，這次嘗試匯入時不要匯入`Year`欄位的資料。

```{r Q5}
Infant_Mortal_skip <- read.csv("../data/infmort.csv",)%>%select(-Year)
```
...
```{r Q7a}
stroop1 <- read.csv("../data/stroop.csv")
library(readxl)
stroop2 <- read_excel("../data/stroop.xlsx")
```
...
> 在反應時間rt的數值上不同，csv資料有4捨5入，excel沒有。
...
```{r Q10}
my_tbl <- tibble(
  A = rep(c("A2","A1"),each = 4,len = 8),
  B = rep(c("B2","B1"),each = 1,len = 8),
  C = rep(c("C3","C4"),each = 4,len = 8)
)
```
...
```{r Q15}
col_skyblue <- colors()[grep("skyblue",colors())]
col_skyblue_length <- length(col_skyblue)
```
