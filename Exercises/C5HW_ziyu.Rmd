...
```{r Q7}
jerry <- usab%>%
  filter(name == "Jerry",year >=1995,year <=2007)%>%
  print()
...
```{r Q9}
murphy <- usab%>%
  filter(name == "Murphy",year >=1990,year <=2005)%>%
  print()
```
...
most_unpopular <- usab%>%
  group_by(name)%>%
  summarise(unpopular = sum(n)) %>%
  filter(rank(unpopular) == min(rank(unpopular))) %>%
  print()
```
...
```{r Q13}
babies_per_year <- usab%>%
  filter(year>=1949)%>%
  group_by(year) %>%
  summarise(n = sum(n))

ggplot(babies_per_year, aes(x = year,y = n)) +
  geom_line()
...
```{r SH1}
id_only <- dataskills::personality %>%
  filter(user_id >= 3000,user_id <= 7000)%>%
  print()
```
...
repeated_data <- fake_data %>%
  group_by(subj_id)%>%
  mutate(same = dv == lag(dv)) %>%
  summarise(repeated = sum(same, na.rm = TRUE))%>%
  print()
```
...
eyes <- dataskills::eye_descriptions%>%
  pivot_longer(t1:t50)%>%
  separate(col = value, into = c("1","2","3","4"),sep = "(~|,)+")%>%
  select(-user_id,-sex,-name,-age)%>%
  gather(d,description,1:4,na.rm = TRUE,)%>%
